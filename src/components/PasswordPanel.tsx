...
export const PasswordPanel = forwardRef<
    PasswordPanelHandle,
    PasswordPanelProps
>(({ onAddPassword, searchQuery = "", onClearSearch = () => {} }, ref) => {
    const [passwords, setPasswords] = useState<ILoginEntry[]>([]);
        ...
        if (query) {
            result = result.filter((password) => {
                const titleMatch = password.title
                    ?.toLowerCase()
                    .includes(query);
                const usernameMatch = password.username
                    ?.toLowerCase()
                    .includes(query);
    ...
    const handleDeletePassword = (passwordToDelete: ILoginEntry) => {
        const updatedPasswords = passwords.filter(
            (p) => p !== passwordToDelete,
        );
    ...
    const handleUpdatePassword = (updatedPassword: ILoginEntry) => {
        ...
        >
            ...
            <div className="flex-1 min-w-0 overflow-hidden">
                <PasswordOverview
                    ...
                    isEditing={selectedPassword !== null}
